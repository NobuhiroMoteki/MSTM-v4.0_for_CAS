...
!27 july: core volume fraction added to output
      ...
      complex(8) :: c_var_start(5),c_var_stop(5),c_var_step(5),effective_ref_index,forward_scat_amp_mat(4),&
         backward_scat_amp_mat(4)
...
call mstm_mpi(mpi_command='barrier')
               if(calculate_scattering_matrix) then
                  call scattering_matrix_calculation(amnp_0,forward_scat_amp_mat,backward_scat_amp_mat,scat_mat,&
                      mpi_comm=mpicomm)
               endif
if(light_up) then
...
call mstm_mpi(mpi_command='barrier')
               ...
               if(calculate_scattering_matrix) then
                  call scattering_matrix_calculation(amnp_s,forward_scat_amp_mat,backward_scat_amp_mat,scat_mat,&
                      mpi_comm=mpicomm)
               endif
               if(gaussian_beam_constant.eq.0.d0) then
...
!            diffac=(1.d0-(1.d0/dble(number_spheres))**.5d0)
            ...
            if(singleorigin) then
               ...
               if(calculate_scattering_matrix) then
                  call scattering_matrix_calculation(amnp_0,forward_scat_amp_mat,backward_scat_amp_mat,dif_scat_mat,&
                      mpi_comm=configcomm)
               endif
               if(azimuthal_average) then
...
!            scaq=0.5d0*(-sum(dif_boundary_sca(:,0))+sum(dif_boundary_sca(:,number_plane_boundaries+1)))
         ...
         subroutine scattering_matrix_calculation(amnp,forward_ampmat,backward_ampmat,scatmat,mpi_comm)
         implicit none
         logical :: singleorigin,iframe
         ...
         real(8) :: scatmat(scat_mat_mdim,scat_mat_ldim:scat_mat_udim),costheta,phi,csca, &
                    ky,kx,sintheta
         complex(8) :: amnp(*),ampmat(4),forward_ampmat(4),backward_ampmat(4)
         if(present(mpi_comm)) then
            mpicomm=mpi_comm
         else
            mpicomm=mpi_comm_world
...
!                        call fosmcalc(12,s00,s02,sp22,sm22,costheta,scatmat(:,i),normalize_s11=.false.)
                        ...
                        call scatteringmatrix(amnp,t_matrix_order,costheta,phi,ampmat,scatmat(:,i),iframe, &
                           normalize_s11=.false.)
                        if(abs(costheta-1) < 1e-4) then
                            forward_ampmat(:)=ampmat(:)
                        endif
                        if(abs(costheta+1) < 1e-4) then
                            backward_ampmat(:)=ampmat(:)
                        endif
                     endif
                  else
            ...
            if(random_orientation) then
               ...
               if(scattering_map_model.eq.0) then
                  if(number_plane_boundaries.eq.0) then
                     ...
                     write(outunit,'('' forward scattering amplitude matrix (BH83,Eq3.12) '')')
                     do i=1,4
                        !write(outunit,'("("e12.4,",",e12.4") ",$)') forward_scat_amp_mat(i)/(-sqrt(4*pi))
                        if (i==4) then
                            write(outunit,'("("e12.4,",",e12.4") ")') forward_scat_amp_mat(i)/(-sqrt(4*pi))
                        else
                            write(outunit,'("("e12.4,",",e12.4") ",$)') forward_scat_amp_mat(i)/(-sqrt(4*pi))
                     ...
                     enddo
                     write(outunit,'('' backward scattering amplitude matrix (BH83,Eq3.12) '')')
                     do i=1,4
                        !write(outunit,'("("e12.4,",",e12.4") ",$)') backward_scat_amp_mat(i)/(-sqrt(4*pi))
                        if (i==4) then
                            write(outunit,'("("e12.4,",",e12.4") ")') backward_scat_amp_mat(i)/(-sqrt(4*pi))
                        else
                            write(outunit,'("("e12.4,",",e12.4") ",$)') backward_scat_amp_mat(i)/(-sqrt(4*pi))
